...
# Backend base URL (change if your FastAPI runs elsewhere)
...
with tab_upload:
    ...
    if st.button("Upload PDF"):
        if not uploaded_file:
            ...
            try:
                # Build files and optional form fields
                files = {
                    # key must match what your backend expects (commonly 'file')
    ...
    if st.button("Send"):
        if not question.strip():
            ...
            payload = {
                "question": question,
                "top_k": top_k,
                # include only if provided (backend should accept None)
            ...
            try:
                ...
                if resp.status_code == 200:
                    data = resp.json()
                    # Try common keys used by backends
                    ...
                    # If backend returns document list (common key: 'results' or 'docs' or 'retrieved_list')
                    ...
                    if docs:
                        ...
                        for i, d in enumerate(docs, 1):
                            # support both dicts returned by weaviate_retrieve or raw objects
                            ...
                            st.write(content[:1000])  # show snippet
                            st.write("---")
                else:
